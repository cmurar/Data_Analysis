...
dates = []
dates = [datetime.datetime(year=int(i[1]), month=int(i[2]), day=1)
         for i in headers]
date_counts = {}
for j in dates:
    if j in date_counts:
...
df = pd.DataFrame(f_read, columns=['month', 'intent', 'sex', 'place',
                  'education'])
df_hom = df[df['intent'].str.contains('Homicide', na=False)]  # creating a df filtered for 'Homicide' intent

homucide_count_month = df_hom[['month', 'intent']].groupby('month'
        ).agg({'intent': 'count'})
homucide_count_gender = df_hom[['sex', 'intent']].groupby('sex'
        ).agg({'intent': 'count'})
death_count_location = df[['place', 'intent']].groupby('place'
        ).agg({'intent': 'count'})
death_count_edu = df[['education', 'intent']].groupby('education'
        ).agg({'intent': 'count'})
